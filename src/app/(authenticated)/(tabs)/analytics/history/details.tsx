...
const HistoryDetailed = () => {
  ...
  const folder_id = parseFloat(typeof folderIdString === 'string' ? folderIdString : folderIdString?.[0])

  const { data: transactionData } = useGetTransaction(folder_id)
    ...
    <Container isPadding={false} container_style='mx-2 pt-2'>
      ...
      <ScrollView contentContainerStyle={{ gap: 10 }}>
        <View className='flex-col'>
          <TransactionDifference first_text='Full name:' second_text={member?.fullName || ''} />
          <TransactionDifference first_text='Item:' second_text={transaction?.changed_item?.name || ''} />
          ...
          {Object.entries(differences).map(([key, value]) => (
            <View key={key} className='mb-2'>
              <TransactionDifference
                first_text={`Prev. ${key.charAt(0).toUpperCase() + key.slice(1)}:`}
                second_text={value.prev}
              />
              <TransactionDifference
                first_text={`New ${key.charAt(0).toUpperCase() + key.slice(1)}:`}
