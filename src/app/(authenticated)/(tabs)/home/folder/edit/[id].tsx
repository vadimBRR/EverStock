...
export default function EditFolder() {
  ...
  useEffect(() => {
    const fetchFolder = async () => {
      ...
      if (current) {
        ...
        setCurrency({ name: current.currency || 'USD', value: '$', countries: [] })
  ...
  useEffect(() => {
    if (type) {
      ...
      if (folderTypeFromParams !== folderType) setFolderType(folderTypeFromParams)
    }
    if (options) {
      const optionsFromParams = Array.isArray(options) ? options : options ? [options] : []
