...
const MemberProfileScreen = () => {
  ...
  const router = useRouter();
	...
	const folder_id = parseFloat(
		folderIdString ? (typeof folderIdString === 'string' ? folderIdString : folderIdString[0]) : ''
	)

  const user = useAccount().folders.find(folder => folder.id === folder_id)?.members.find(member => member.id === user_id)


  

	const [email, setEmail] = useState(user?.email || '')
	const [roles, setRoles] = useState(user?.roles || {isView: false, isAddItem: false, isDeleteItem: false, isEdit: false, isCanInvite: false, isAdmin: false})
	if (!user_id) {
  ...
  const updateMember = () => {
    ...
    handleUpdateMember({folderId: folder_id,email, roles: roles, id: user_id})
    router.back()
